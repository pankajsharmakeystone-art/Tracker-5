...
import { appendCandidate, createViewerRequest, endLiveSession, getLiveSessionRef, setAnswer, type LiveSessionCandidate, type LiveSessionDoc } from '../services/liveSessions';
import { getRtcConfiguration } from '../utils/liveStream';

export type ViewerState = 'idle' | 'requesting' | 'waiting' | 'connecting' | 'streaming' | 'ended' | 'error';
        ...
        const unsubscribe = onSnapshot(sessionRef, async (snapshot) => {
          ...
          if (pcRef.current && data.agentIceCandidates?.length) {
            for (const candidate of data.agentIceCandidates) {
              if (!candidate?.id || processedAgentCandidates.current.has(candidate.id)) continue;
