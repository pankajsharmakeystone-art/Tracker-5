...
export const captureDesktopStreamsForLive = async (): Promise<DesktopCaptureResult[]> => {
  ...
  const screenSources = sourceResult.sources.filter((src: any) => src.id?.startsWith('screen'));
  const targets = screenSources.length > 0 ? screenSources : [sourceResult.sources[0]];

  const captures: DesktopCaptureResult[] = [];
...
export const stopMediaStreamTracks = (stream: MediaStream | MediaStream[] | null) => {
