import { doc, setDoc, getDoc, serverTimestamp, collection, query, where, getDocs, addDoc, limit, updateDoc, Timestamp, increment, onSnapshot, deleteField } from 'firebase/firestore';
...
export const createUserDocument = async (userAuth, additionalData) => {
    ...
    if (!snapshot.exists()) {
        ...
        let initialTeamIds = additionalData.teamIds || [];
...
export const streamTeamsForAdmin = (adminId, callback) => {
...
export const getAllUsers = async () => {
    ...
    return querySnapshot.docs.map(doc => {
        ...
        if (!data.teamIds && data.teamId)
            teamIds = [data.teamId];
        return { uid: doc.id, ...data, teamIds };
    });
};
export const streamAllUsers = (callback) => {
    const usersCollectionRef = collection(db, 'users');
    const q = query(usersCollectionRef);
    return onSnapshot(q, (querySnapshot) => {
...
export const getUsersByTeam = async (teamId) => {
    ...
    try {
        ...
        const processDoc = (doc) => {
            ...
            if (!data.teamIds && data.teamId)
                teamIds = [data.teamId];
            usersMap.set(doc.id, { uid: doc.id, ...data, teamIds });
        };
...
export const streamUsersByTeam = (teamId, callback) => {
    const usersCollectionRef = collection(db, 'users');
 ...
 */
export const createSessionLogId = (uid, date) => {
 ...
 */
export const getActiveWorkLog = async (uid) => {
    const logsRef = collection(db, 'worklogs');
    const q = query(logsRef, where("userId", "==", uid), where("status", "in", ["working", "on_break", "break"]));
 ...
 */
export const streamActiveWorkLog = (uid, callback) => {
    const logsRef = collection(db, 'worklogs');
    ...
    return onSnapshot(q, (snapshot) => {
        if (!snapshot.empty) {
            ...
            callback({ id: doc.id, ...data, status: normalizedStatus });
        }
        else {
            callback(null);
        }
 ...
 */
export const forceCloseLog = async (logId, endTime) => {
 ...
 */
export const performClockIn = async (uid, teamId, userDisplayName) => {
    const now = new Date();
 ...
 */
export const performClockOut = async (uid) => {
    const activeLog = await getActiveWorkLog(uid);
    if (!activeLog) {
    ...
    let workDuration = 0;
    let breakDuration = 0;
    const getMillis = (ts) => (ts?.toMillis ? ts.toMillis() : (ts?.toDate ? ts.toDate().getTime() : Date.now()));
    if (activeLog.lastEventTimestamp) {
        ...
        else if (activeLog.status === 'on_break' || activeLog.status === 'break') {
...
export const updateWorkLog = async (logId, data) => {
 ...
 */
export const streamTodayWorkLogs = (callback, teamId) => {
    const logsRef = collection(db, "worklogs");
    const startOfToday = new Date();
    startOfToday.setHours(0, 0, 0, 0);
    const startOfTodayTimestamp = Timestamp.fromDate(startOfToday);
    // Query A: Started Today (Active OR Closed)
    const qToday = query(logsRef, where("date", ">=", startOfTodayTimestamp));
    // Query B: Currently Active (Started anytime)
    // Note: This might return docs already in qToday, we must dedupe.
    const qActive = query(logsRef, where("status", "in", ["working", "on_break", "break"]));
    let todayLogs = [];
    let activeLogs = [];
    const emit = () => {
        const merged = new Map();
    ...
    const unsubToday = onSnapshot(qToday, (snap) => {
        todayLogs = snap.docs.map(doc => {
            ...
            const rawStatus = data.status;
            const normalizedStatus = rawStatus === 'break' ? 'on_break' : rawStatus;
            return { id: doc.id, ...data, status: normalizedStatus };
        });
        emit();
    });
...
export const streamWorkLogsForDate = (dateString, callback, teamId) => {
    ...
    const logsRef = collection(db, "worklogs");
    const q = query(logsRef, where("date", ">=", startTs), where("date", "<=", endTs));
...
export const getWorkLogsForDateRange = async (teamId, startDate, endDate) => {
    const logsCollectionRef = collection(db, 'worklogs');
    const startTs = Timestamp.fromDate(startDate);
    const endTs = Timestamp.fromDate(endDate);
    const q = query(logsCollectionRef, where("teamId", "==", teamId), where("date", ">=", startTs), where("date", "<=", endTs));
    const querySnapshot = await getDocs(q);
    return querySnapshot.docs.map(doc => {
        const data = doc.data();
        const rawStatus = data.status;
        const normalizedStatus = rawStatus === 'break' ? 'on_break' : rawStatus;
        return { id: doc.id, ...data, status: normalizedStatus || 'clocked_out' };
    });
};
// --- Helpers for Stale/Zombie Handling ---
export const isSessionStale = (log) => {
    // Logic: If user hasn't pinged in > 4 hours (zombie) OR is from previous day and not overnight
    if (!log.lastEventTimestamp)
        return false;
    const now = Date.now();
    const lastTime = log.lastEventTimestamp.toMillis ? log.lastEventTimestamp.toMillis() : log.lastEventTimestamp.toDate().getTime();
    const diffHours = (now - lastTime) / (1000 * 60 * 60);
    if (diffHours > 4)
        return true; // Zombie detection
    return false;
};
export const closeStaleSession = async (log) => {
    console.log("Closing stale session:", log.id);
    // Use lastEventTimestamp as the cut-off
    await forceCloseLog(log.id, log.lastEventTimestamp.toDate());
    await updateAgentStatus(log.userId, 'offline');
};
// --- Scheduling & Admin (Existing) ---
export const getScheduleForMonth = async (teamId, year, month) => {
...
export const streamScheduleForMonth = (teamId, year, month, callback) => {
    const monthStr = `${year}-${String(month).padStart(2, '0')}`;
    const scheduleDocRef = doc(db, 'schedules', `${teamId}-${monthStr}`);
...
export const updateScheduleForMonth = async (teamId, year, month, scheduleData) => {
    const monthStr = `${year}-${String(month).padStart(2, '0')}`;
    const scheduleDocRef = doc(db, 'schedules', `${teamId}-${monthStr}`);
    await setDoc(scheduleDocRef, scheduleData, { merge: true });
};
export const streamGlobalAdminSettings = (callback) => {
...
export const updateGlobalAdminSettings = async (settings) => {
    const settingsRef = doc(db, 'adminSettings', 'global');
    await setDoc(settingsRef, settings, { merge: true });
};
export const updateAgentAutoClockOut = async (uid, config) => {
    const docRef = doc(db, 'adminSettings', 'agents');
    await setDoc(docRef, { [uid]: { autoClockOut: config } }, { merge: true });
};
export const updateAgentStatus = async (uid, status, additionalData = {}) => {
...
export const streamAllAgentStatuses = (callback) => {
...
export const sendCommandToDesktop = async (uid, command) => {
